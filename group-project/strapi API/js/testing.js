...
async function getDataFromStrapi() {
  ...
  //Url till Strapi.js API form sida laptops
  ...
  if (Array.isArray(myObjekt.data) && myObjekt.data.length > 0) {
    ...
    // en variabel med en path till attribut i objectet
...
async function postData() {
  ...
  const urlProduct = "http://localhost:1337/api/laptops/";

  // Hämtar data från fält i .html
  const title = document.getElementById("title").value;
  const description = document.getElementById("description").value;
  const price = document.getElementById("price").value;
  const qty = document.getElementById("qty").value;

  //Skapa ett objekt med data från .html fil.
  let productObjekt = {
    data: {
      title: title,
...
function generateRow(obj, arr, objId, header,) {
  let output = "<tr>";
  ...
  if (!header && obj) {
    ...
    let postURL = `http://localhost:1337/api/${arr[i]}/${objId}`;
...
async function updatePost(url) {
  ...
  //Fyller upp Data med parameter-värden från .html till databas genom att spara den i "productObjekt"
